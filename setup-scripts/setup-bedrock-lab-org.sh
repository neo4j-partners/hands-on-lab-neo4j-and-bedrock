...
create_role() {
    local account_id="$1"
    echo -e "${YELLOW}[1/4] Creating IAM Role: ${ROLE_NAME}${NC}"
...
create_policy() {
    local account_id="$1"
    echo -e "${YELLOW}[2/4] Creating IAM Policy: ${POLICY_NAME}${NC}"
...
attach_policy() {
    local account_id="$1"
    echo -e "${YELLOW}[3/4] Attaching policy to role${NC}"
...
create_inference_profiles() {
    local account_id="$1"
    echo -e "${YELLOW}[4/4] Creating Inference Profiles${NC}"
